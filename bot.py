...
async def resync_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionFactory() as db:
        user = await db.get(User, user_id)
    if not user:
        await update.message.reply_text("No user record.")
        return
    wallets = []
    if user.ton_wallet:
        wallets.append(f"TON: {user.ton_wallet}")
    if user.evm_wallet:
        wallets.append(f"EVM: {user.evm_wallet}")
    if user.sol_wallet:
        wallets.append(f"Solana: {user.sol_wallet}")
    if wallets:
        update_context(user_id, {"state": None, "data": {}, "history": get_context(user_id).get('history', []) + ['resync_wallet']})
        await update.message.reply_text("🔄 Wallets synced:\n" + "\n".join(wallets))
    else:
